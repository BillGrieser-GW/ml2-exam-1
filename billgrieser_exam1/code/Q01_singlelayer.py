...
# Instantiate a model
net = NetAlt(input_size, hidden_size, num_classes)
print(net)
...
run_base=os.path.join('results', 'console')

if sys.argv[0] != '':
    run_base = os.path.basename(sys.argv[0])
    run_base = os.path.join("results", os.path.splitext(run_base)[0])
    
torch.save(net.state_dict(), run_base + '.pkl')

with open(run_base + '_results.txt', 'w') as rfile:
    rfile.write(str(net))
    rfile.write('\n\n')
